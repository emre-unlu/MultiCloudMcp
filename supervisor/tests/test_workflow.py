...
class WorkflowTests(unittest.TestCase):
    ...
    def test_detection_handles_list_pods_tool_shape_and_restarts(self):
        ...
        self.assertEqual(result["diagnostics_summary"]["detection"]["unhealthy_pods"], ["default/api-0"])


    def test_detection_parses_stringified_list_pods_payload(self):
        workflow = SupervisorWorkflow(
            operations_agent=self.ops,
            diagnostics_tools={
                "list_pods": lambda namespace="default": '[{"name":"api-1","phase":"CrashLoopBackOff"}]',
                "pod_events": lambda namespace, pod: {},
                "pod_logs": lambda namespace, pod, tail_lines=80: {},
                "exec_shell": lambda command, timeout=30: {},
            },
        ...
        result = workflow.detection_node({"user_request": "diagnose namespace default"})
        self.assertTrue(result["issue_found"])
        self.assertEqual(result["diagnostics_summary"]["detection"]["unhealthy_pods"], ["default/api-1"])
